@ModelType PagedList.IPagedList(Of Theme)
@Imports PagedList.Mvc
@Imports iMeeting.My.Resources

@Code
    ViewData("Title") = Resource.theme_ListTitle				
End Code

			

<h2>@Resource.theme_ListTitle</h2>

@Using (Html.BeginForm("Index", "Themes", FormMethod.Get, New With {.class = "form-inline", .role = "form"}))
...
<thead>
    <tr>
			...
			<th>
				@Html.ActionLink(Resource.theme_Libelle, "Index", New With {.sortOrder = ViewBag.LibelleSortParm, .currentFilter = ViewBag.CurrentFilter, .tab = ViewBag.activeTab})
			</th>
