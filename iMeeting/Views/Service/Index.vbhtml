@ModelType PagedList.IPagedList(Of SERVICE)
@Imports PagedList.Mvc
@Imports iMeeting.My.Resources
@Code
    ViewData("Title") = Resource.serv_ListTitle
End Code

<h2>@Resource.serv_ListTitle</h2>

@Using (Html.BeginForm("Index", "Service", FormMethod.Get, New With {.class = "form-inline", .role = "form"}))
...
<table class="table table-responsive table-bordered table-striped table-condensed">
    <thead>
        <tr>
            ...
            <th>
                @Html.ActionLink(Resource.serv_IdBat, "Index", New With {.sortOrder = ViewBag.IdBatSortParm, .currentFilter = ViewBag.CurrentFilter})
            </th>
            <th class="hidden-xs">
                @Html.ActionLink(Resource.serv_Libelle, "Index", New With {.sortOrder = ViewBag.LibelleSortParm, .currentFilter = ViewBag.CurrentFilter})
            </th>
            <th class="hidden-xs">
                @Html.ActionLink(Resource.serv_Details, "Index", New With {.sortOrder = ViewBag.DetailsSortParm, .currentFilter = ViewBag.CurrentFilter})
            </th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        @For Each item In Model
            @<tr>
                ...
                <td>
                    <a class="btn btn-primary btn-sm" title="@Resource.Edit" href="@Url.Action("Edit", New With {.id = item.ID_SERVICE})">
                        <span class="glyphicon glyphicon-pencil"></span>
                        <span class="sr-only">@Resource.Edit</span>
                    </a>
                    <a class="btn btn-primary btn-sm" title="@Resource.Details" href="@Url.Action("Details", New With {.id = item.ID_SERVICE})">
                        <span class="glyphicon glyphicon-list-alt"></span>
                        <span class="sr-only">@Resource.Details</span>
                    </a>
                    <a class="btn btn-primary btn-sm" title="@Resource.Delete" href="@Url.Action("Delete", New With {.id = item.ID_SERVICE})">
